...
namespace FSPlay.GameEngine.Network
{
    ...
    public class TCPGame
    {
        ...
        /// <summary>
        /// Gửi yêu cầu nào đó trong ott với đối phương
        /// </summary>
        /// <param name="otherRoleID"></param>
        /// <param name="exchangeType"></param>
        /// <param name="exchangeID"></param>
        public void SpriteGoodsOTT(int otherRoleID, int exchangeType, int exchangeID)
        {
            if (!Global.Data.PlayGame)
            {
                return;
            ...
            string strcmd = "";
            strcmd = string.Format("{0}:{1}:{2}:{3}", CurrentSession.RoleID, otherRoleID, exchangeType, exchangeID);
            tcpClient.SendData(TCPOutPacket.MakeTCPOutPacket(tcpClient.OutPacketPool, strcmd, (int)(TCPGameServerCmds.CMD_SPR_GOODSOTT)));
        }
        ...
        public void SendMergeGem(int GoodId, int type)
        {
            string strCmd = string.Format("{0}", GoodId);
            byte[] bytes = new ASCIIEncoding().GetBytes(strCmd);
            if (type == 1)
            {
                tcpClient.SendData(TCPOutPacket.MakeTCPOutPacket(tcpClient.OutPacketPool, bytes, 0, bytes.Length, (int)(TCPGameServerCmds.CMD_KT_CLIENT_MERGE_GEM)));
            }
            else if (type == 2)
            {
                tcpClient.SendData(TCPOutPacket.MakeTCPOutPacket(tcpClient.OutPacketPool, bytes, 0, bytes.Length, (int)(TCPGameServerCmds.CMD_KT_CLIENT_MERGE_GEM_ALL)));
            }
