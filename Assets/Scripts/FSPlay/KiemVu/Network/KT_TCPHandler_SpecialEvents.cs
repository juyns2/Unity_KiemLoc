...
namespace FSPlay.KiemVu.Network
{
    ...
    public static partial class KT_TCPHandler
    {
        ...
        /// <summary>
        /// Gửi yêu cầu thông tin dữ liệu thi đấu loạn phái
        /// </summary>
        public static void SendGetTypeOfMatchData()
        {
            string strCmd = string.Format("");
            byte[] bytes = new ASCIIEncoding().GetBytes(strCmd);
            GameInstance.Game.GameClient.SendData(TCPOutPacket.MakeTCPOutPacket(GameInstance.Game.GameClient.OutPacketPool, bytes, 0, bytes.Length, (int)(TCPGameServerCmds.CMD_KT_TYPEOFMATCH_PVP_RANKING_INFO)));
        }

        /// <summary>
        /// Nhận gói tin thông tin dữ liệu thi đấu loạn phái
        /// </summary>
        /// <param name="cmdID"></param>
        /// <param name="bytes"></param>
        /// <param name="length"></param>
        public static void ReceiveTypeOfMatchData(int cmdID, byte[] bytes, int length)
        {
            FACTION_PVP_RANKING_INFO_1 rankingInfo = DataHelper.BytesToObject<FACTION_PVP_RANKING_INFO_1>(bytes, 0, length);
            if (rankingInfo == null)
            {
                KTGlobal.AddNotification("Bảng xếp hạng chưa cập nhật!");
                return;
            ...
            if (rankingInfo.State == 0)
            {
                PlayZone.GlobalPlayZone.OpenUITypeOfMatchRanking(rankingInfo.PlayerRanks);
            }
            /// Hiển thị thi đấu loạn phái
            else if (rankingInfo.State == 1)
            {
                PlayZone.GlobalPlayZone.OpenUITypeOfMatchRoundInfo(rankingInfo);
            }
        }
        #endregion
