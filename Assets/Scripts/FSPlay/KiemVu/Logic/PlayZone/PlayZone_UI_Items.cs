...
public partial class PlayZone
{
    ...
    public void ShowUIAdvertiseItem(GoodsData itemGD)
    {
        ...
        {
            GameInstance.Game.SpriteSendChat(Global.Data.RoleData.RoleName, toRoleName, content, channel, new List<GoodsData>() { itemGD });
        };
    }
    ...
    public void OpenUIEnhance()
    {
        ...
        this.UIEnhance.Split = (itemGD, number) =>
        {
    ...
    public void OpenUICrystalStoneSynthesis()
    {
        ...
        this.UICrystalStoneSynthesis.Split = (itemGD, number) =>
        {
            GameInstance.Game.SpriteModGoods((int)ModGoodsTypes.SplitItem, itemGD.Id, itemGD.GoodsID, -1, itemGD.Site, itemGD.GCount - number, itemGD.BagIndex, number.ToString());
            KTGlobal.CloseItemInfo();
        };
    ...
    public void OpenUISplitCrystalStone(int minLevelToSplit, float nRate)
    {
        ...
        this.UISplitCrystalStone.Splits = (itemGD, number) =>
        {
            GameInstance.Game.SpriteModGoods((int)ModGoodsTypes.SplitItem, itemGD.Id, itemGD.GoodsID, -1, itemGD.Site, itemGD.GCount - number, itemGD.BagIndex, number.ToString());
            KTGlobal.CloseItemInfo();
        };
    ...
    #region Khung Bảo thạch
    ///<summary>
    ///Khung bảo thạch
    ///</summary>
    public UIGemStone UIGemStone { get; protected set; }
    /// <summary>
    /// Mở khung khảm bảo thạch
    /// </summary>
    public void OpenUIGemStone()
    {
        if (this.UIGemStone != null)
        {
            return;
        }
        this.UIGemStone = CanvasManager.Instance.LoadUIPrefab<UIGemStone>("MainGame/GemStone/UIGemStone");
        CanvasManager.Instance.AddUI(this.UIGemStone);

        this.UIGemStone.Close = this.CloseUIGemStone;

    }

    /// <summary>
    /// Đóng khung khảm bảo thạch
    /// </summary>
    public void CloseUIGemStone()
    {
        if (this.UIGemStone != null)
        {
            GameObject.Destroy(this.UIGemStone.gameObject);
            this.UIGemStone = null;
        }
    }
    #endregion
}
