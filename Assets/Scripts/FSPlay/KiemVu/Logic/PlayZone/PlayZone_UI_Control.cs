...
public partial class PlayZone
{
    ...
    protected void InitTopFunctionButtons()
    {
        ...
        this.UITopFunctionButtons.OpenSeashellCircle = () => {
            /// Gửi gói tin yêu cầu truy vấn thông tin Bách bảo rương
            KT_TCPHandler.SendOpenSeashellCircle();
        };
		...
		this.UITopFunctionButtons.OpenGiftCode = () => {
            this.OpenUIGiftCode();
        };
		this.UITopFunctionButtons.OpenLuckyCircle = () => {
            KT_TCPHandler.SendOpenLuckyCircle();
        };
        this.UITopFunctionButtons.OpenPlayerPray = () => {
            KT_TCPHandler.SendOpenPlayerPray();
        };
    }
    #endregion
    ...
    protected void InitBottomBar()
    {
		...
		this.UIBottomBar.MenuFaceSelected = (isSelected) => {
            if (this.UIBottomBar == null)
            {
                return;
            }
    ...
    protected void InitSkillBar()
    {
        ...
        this.UIBottomBar.UISkillBar.AutoFarm = () => {
            if (!this.UIBottomBar.UISkillBar.AutoFarmEnable || this.UIBottomBar.UISkillBar.AutoPKEnable)
            {
        ...
        this.UIBottomBar.UISkillBar.AutoPK = () => {
            if (!this.UIBottomBar.UISkillBar.AutoPKEnable || this.UIBottomBar.UISkillBar.AutoFarmEnable)
            {
                KTAutoFightManager.Instance.StopAutoFight();
		...
		this.UIBottomBar.UISkillBar.OpenBag = () => {
            this.ShowUIBag();
        };
        this.UIBottomBar.UISkillBar.FixLag = (isSelected) => {
            Global.Data.FixLag = isSelected;

            if (isSelected)
            {
                /// Ẩn các hiệu ứng
                KTSystemSetting.HideSkillCastEffect = true;
                KTSystemSetting.HideSkillExplodeEffect = true;
                KTSystemSetting.HideSkillBuffEffect = true;
                KTSystemSetting.DisableTrailEffect = true;
                KTSystemSetting.HideWeaponEnhanceEffect = true;
                KTSystemSetting.EffectQualitySetting = 0;
                ...
                KTGlobal.AddNotification("Mở chức năng FixLag");
            ...
            else
            {
                ...
                KTSystemSetting.EffectQualitySetting = 1;
                ...
                KTGlobal.AddNotification("Tắt chức năng FixLag");
    ...
    protected void InitControlButtons()
    {
        ...
        this.UIBottomBar.UIControlButtons.OpenGuildManager = () => {
            ...
            {
                KTGlobal.AddNotification("Bạn chưa có bang hội!");
                return;
            }
            /// Gửi yêu cầu truy vấn thông tin bang hội
            KT_TCPHandler.SendGetGuildInfo();
        };
        this.UIBottomBar.UIControlButtons.OpenFamilyManager = () => {
            /// Nếu chưa có gia tộc
            if (Global.Data.RoleData.FamilyID <= 0)
            {
                KTGlobal.AddNotification("Bạn chưa có gia tộc!");
                return;
            }
            /// Gửi yêu cầu truy vấn thông tin gia tộc
            KT_TCPHandler.SendToSeverOpenFamily();
        };
    ...
    private void ClearGameData(int nClearType = 0)
    {
        if (0 == nClearType)
        {
    ...
    #region Màn hình chào lúc vào Game
    /// <summary>
    /// Khung chào mừng vào Game
    /// </summary>
    protected UIWelcomeDialog UIWelcomeDialog { get; set; } = null;

    /// <summary>
    /// Đóng màn hình chào mừng
    /// </summary>
    public void CloseWelcomeDialog()
    {
        ...
        {
            CanvasManager canvas = Global.MainCanvas.GetComponent<CanvasManager>();
            canvas.RemoveUI(this.UIWelcomeDialog);
            this.UIWelcomeDialog = null;
        }
    }
    ...
    {
        CanvasManager canvas = Global.MainCanvas.GetComponent<CanvasManager>();
        this.UIWelcomeDialog = canvas.LoadUIPrefab<UIWelcomeDialog>("MainGame/WelcomeDialog/UIWelcomeDialog");
        if (this.UIWelcomeDialog == null)
        {
            return;
        }

        canvas.AddUI(this.UIWelcomeDialog, true);
        this.UIWelcomeDialog.OnOK = () => {
