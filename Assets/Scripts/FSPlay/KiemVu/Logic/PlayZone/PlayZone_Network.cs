...
public partial class PlayZone
{
    ...
    protected void ProcessNetCommand(SocketConnectEventArgs e)
    {
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_G2C_UPDATE_OTHERROLE_EQUIP)
        {
            ...
            if (Global.Data.OtherRoles.TryGetValue(roleDataMini.RoleID, out RoleData rd))
            {
				...
				GoodsData channguyen = GetItemGD(roleDataMini.ChanNguyenID);
                if (channguyen != null)
                {
                    channguyen.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_CHANNGUYEN;
                    rd.GoodsDataList.Add(channguyen);
                }
				GoodsData ngoaitrang = GetItemGD(roleDataMini.NgoaiTrangID);
                if (ngoaitrang != null)
                {
                    ngoaitrang.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_NGOAITRANG;
                    rd.GoodsDataList.Add(ngoaitrang);
                }
				GoodsData thanbinh = GetItemGD(roleDataMini.ThanBinhID);
                if (thanbinh != null)
                {
                    thanbinh.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_THANBINH;
                    rd.GoodsDataList.Add(thanbinh);
                }
        ...
        else if (e.CmdID == (int)(TCPGameServerCmds.CMD_OTHER_ROLE))
        {
            ...
            if (roleDataMini != null)
            {
                ...
                {
                    channguyen.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_CHANNGUYEN;
                    roleData.GoodsDataList.Add(channguyen);
                }
				GoodsData ngoaitrang = GetItemGD(roleDataMini.NgoaiTrangID);
                if (ngoaitrang != null)
                {
                    ngoaitrang.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_NGOAITRANG;
                    roleData.GoodsDataList.Add(ngoaitrang);
                }
				GoodsData thanbinh = GetItemGD(roleDataMini.ThanBinhID);
                if (thanbinh != null)
                {
                    thanbinh.Using = (int)KE_EQUIP_POSITION.emEQUIPPOS_THANBINH;
                    roleData.GoodsDataList.Add(thanbinh);
                }
            }
        ...
        else if (e.CmdID == (int)(TCPGameServerCmds.CMD_SPR_GOODSOTT))
        {
            if (!AssertNetworkException(e.fields.Length >= 3, e.fields.Length, (TCPGameServerCmds)e.CmdID))
            {
                return;
            }
            this.ProcessGoodsOTT(e.fields);
        }
        else if (e.CmdID == (int)(TCPGameServerCmds.CMD_SPR_OTTDATA))
        {
            Global.Data.ExchangeDataItem = DataHelper.BytesToObject<ExchangeData>(e.bytesData, 0, e.bytesData.Length);
            if (Global.Data.ExchangeDataItem != null)
            {
                if (this.UIOTT != null)
                {
                    this.UIOTT.RefreshExchangeData();
                }
            }
        }
        ...
        else if (e.CmdID == (int)(TCPGameServerCmds.CMD_SPR_UPDATE_ROLEDATA))
        {
            ...
            Int64 maxHP = lifeChangeData.MaxHP;     // int default HP
            int maxMP = lifeChangeData.MaxMP;
            int maxStamina = lifeChangeData.MaxStamina;
            Int64 hp = lifeChangeData.HP;           // int default HP
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_G2C_RENEW_SKILLLIST)
        {
            ...
            UnityEngine.Debug.Log("<==================================== PlayZone_Network (CMD_KT_G2C_RENEW_SKILLLIST)===================================>");
        }
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_C2G_USESKILL)
        {
            ...
            UnityEngine.Debug.Log("<==================================== PlayZone_Network (CMD_KT_C2G_USESKILL) ===================================>");
        }
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_G2C_USESKILL)
        {
            ...
            UnityEngine.Debug.Log("<==================================== (CMD_KT_C2G_USESKILL) PlayZone_Network ===================================>");
        }
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_SPR_GET_REPAYACTIVEAWARD)
        {
            ...
            //UnityEngine.Debug.Log(nRetValue);
            // First To End 250114
            if (result == -3)
            {
                KTGlobal.ShowMessageBox("Thông báo", "Bạn chưa nhận mốc thưởng phía trên kìa!\nBạn phải nhận thưởng ở mốc đầu tiên từ trên xuống dưới!", () => {
                }, false);
            }
            else
            {
        ...
        else if (e.CmdID == (int)(TCPGameServerCmds.CMD_KT_TYPEOFMATCH_PVP_RANKING_INFO))
        {
            KT_TCPHandler.ReceiveTypeOfMatchData(e.CmdID, e.bytesData, e.bytesData.Length);
        }
        ...
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_CLIENT_DRILL_HOLE_ITEM)
        {
            KT_TCPHandler.ReceiveDrillGem(e.fields);
            KTDebug.Log("CMD_KT_CLIENT_DRILL_HOLE_ITEM");
        }
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_CLIENT_ATTACH_GEM)
        {
            KT_TCPHandler.ReceiveAttachGem(e.fields);
            KTDebug.Log("CMD_KT_CLIENT_ATTACH_GEM");
        }
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_CLIENT_REMOVE_GEM)
        {
            KT_TCPHandler.ReceiveRemoveGem(e.fields);
            KTDebug.Log("CMD_KT_CLIENT_REMOVE_GEM");
        }
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_CLIENT_MERGE_GEM)
        {
            KTDebug.Log("CMD_KT_CLIENT_MERGE_GEM");
        }
        else if (e.CmdID == (int)TCPGameServerCmds.CMD_KT_CLIENT_MERGE_GEM_ALL)
        {
            KTDebug.Log("CMD_KT_CLIENT_MERGE_GEM_ALL");
        }
        #endregion
        #endregion
    }
    ...
    #region Oẳn Tù Ti
    public void ProcessGoodsOTT(string[] fields)
    {
        try
        {
            int ret = Convert.ToInt32(fields[0]);
            ...
            if (Global.Data.RoleData.RoleID == roleID)
            {
                ...
                if (exchangeType == (int)(GoodsExchangeCmds.Request))
                {
                    if (ret < 0)
                    {
                        if (-1 == ret)
                        {
                            KTGlobal.AddNotification("Đối phương đã Offline, không thể thực hiện Oẳn Tù Tì!");
                        }
                        else if (-2 == ret)
                        {
                            KTGlobal.AddNotification("Đối phương đang thực hiện Oẳn Tù Tì cùng người khác!");
                        }
                        else if (-3 == ret || -4 == ret)
                        {
                            KTGlobal.AddNotification("Khoảng cách đến đối phương quá xa!");
                        ...
                        {
                            KTGlobal.AddNotification("Đã gửi yêu cầu Oẳn Tù Tì đến đối phương trước đó, hãy kiên nhẫn chờ phản hồi!");
                        }
                        else
                        {
                            KTGlobal.AddNotification(string.Format("Gửi yêu cầu Oẳn Tù Tì thất bại, mã lỗi: {0}!", ret));
                        }
                    }
                    else
                    {
                ...
                else if (exchangeType == (int)(GoodsExchangeCmds.Agree))
                {
                    if (ret < 0)
                    {
                        if (-1 == ret)
                        {
                            KTGlobal.AddNotification("Đối phương đã Offline, không thể Oẳn Tù Tì!");
                        }
                        else if (-2 == ret)
                        {
                            KTGlobal.AddNotification("Đối phương đang thực hiện Oẳn Tù Tì cùng người khác!");
                        ...
                        {
                            KTGlobal.AddNotification(string.Format("Oẳn Tù Tì thất bại, mã lỗi: {0}!", ret));
                        }
                    }
                    else
                    {
                        /// Mở khung giao dịch
                        this.ShowUIOTT(otherRoleID);
                    }
                }
                /// Nếu là từ chối
                else if (exchangeType == (int)(GoodsExchangeCmds.Cancel))
                ...
                {
                    KTGlobal.AddNotification(string.Format("Bạn đã hủy xác nhận Oẳn Tù Tì với {0}", roleData.RoleName));
                }
                /// Nếu là hoàn tất giao dịch
                else if (exchangeType == (int)(GoodsExchangeCmds.Done))
                {
                    ...
                    if (ret >= 0)
                    {
                        KTGlobal.AddNotification("Oẳn Tù Tì thành công!");
                    }
                    else
                    {
                        if (-1 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi đồ của đối phương đã đầy!");
                        }
                        else if (-1001 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi đồ của đối phương đã đầy!");
                        ...
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi đồ của bạn đã đầy!");
                        }
                        else if (-1011 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi đồ của bạn đã đầy!");
                        ...
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, số bạc mang theo không đủ!");
                        }
                        else if (-12 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, số bạc của đối phương mang theo không đủ!");
                        }
                        else if (-1000 == ret)
                        {
                            KTGlobal.AddNotification("OTT thất bại, 1 trong 2 người chưa chọn!");
                        }
                    }
                }
            }
            else
            {
                ...
                if (exchangeType == (int)(GoodsExchangeCmds.Request))
                {
                    ...
                    KTGlobal.ShowMessageBox("Yêu cầu Oẳn Tù Tì", string.Format("<color=#47b9ff>{0}</color> muốn Oẳn Tù Tì cùng bạn, xác nhận không?", roleData.RoleName), () =>
                    {
                        Global.Data.ExchangeID = ret;
                        GameInstance.Game.SpriteGoodsOTT(roleID, (int)(GoodsExchangeCmds.Agree), ret);
                    }, () =>
                    {
                        GameInstance.Game.SpriteGoodsOTT(roleID, (int)(GoodsExchangeCmds.Refuse), ret);
                    });
                }
                /// Nếu là từ chối giao dịch
                else if (exchangeType == (int)(GoodsExchangeCmds.Refuse))
                {
                    KTGlobal.AddNotification("Đối phương từ chối Oẳn Tù Tì cùng bạn!");
                }
                /// Nếu là đồng ý giao dịch
                else if (exchangeType == (int)(GoodsExchangeCmds.Agree))
                {
                    if (ret < 0)
                    {
                        ...
                        {
                            KTGlobal.AddNotification("Đối phương đang tiến hành Oẳn Tù Tì cùng người khác!");
                        }
                        else if (-3 == ret || -4 == ret)
                        {
                            KTGlobal.AddNotification("Khoảng cách quá xa!");
                        ...
                        {
                            KTGlobal.AddNotification("Bạn đang thực hiện Oẳn Tù Tì, không thể thực hiện thêm!");
                        }
                        else
                        {
                            KTGlobal.AddNotification(string.Format("Oẳn Tù Tì thất bại, mã lỗi: {0}!", ret));
                ...
                else if (exchangeType == (int)(GoodsExchangeCmds.Cancel))
                {
                    ...
                    KTGlobal.AddNotification("Đối phương đã hủy Oẳn Tù Tì!");
                }
                /// Nếu là thêm vật phẩm
                else if (exchangeType == (int)(GoodsExchangeCmds.AddGoods))
                {
                ...
                else if (exchangeType == (int)(GoodsExchangeCmds.Done))
                {
                    ...
                    else
                    {
                        if (-1 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi của bạn đã đầy!");
                        }
                        else if (-11 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, túi của đối phương đã đầy!");
                        }
                        else if (-2 == ret)
                        {
                            KTGlobal.AddNotification("Oẳn Tù Tì thất bại, số bạc của đối phương mang theo không đủ!");
