...
namespace FSPlay.KiemVu.Logic
{
    ...
    public partial class KTAutoFightManager
    {
        ...
        /// </summary>
        /// <returns></returns>
        private IEnumerator ProcessAutoFight(bool IsTrainMonster = true)
        {
            /// Thiết lập loại Auto
            KTAutoAttackSetting.IsTrainMode = IsTrainMonster;
            ...
            startPos = leader.PositionInVector2;
            ...
            while (true)
            {
                ...
                /// Nếu bị khống chế hoặc đang chuyển bản đồ thì th
                ...
                this.AutoActivatePositiveBuffs();
                ...
                if (!leader.IsMoving && Global.Data.Leader.IsReadyToMove && KTGlobal.FinishedUseSkillAction)
                {
                    ...
                    // Nếu đang bán đồ hoặc đang đốt lửa trại thì bỏ qua hết các thao tác khác
                    if (!this.DoingAutoSell && !this.DoingAutoFireCamp)
                    {
                        ...
                        if (gp != null)
                        {
                            ...
                            this.ListIgnoreGoodsPack.Add(gp.AutoID);
                        ...
                        if (KTAutoAttackSetting.IsTrainMode && !IsTriger)
                        {
                            if(KTAutoAttackSetting._AutoConfig._AutoTrainConfig.UseSkillByCombo)
                            {
                            ...
                            {
                                /// Chọn 1 kỹ năng không trong trạng thái phục hồi và thỏa mãn điều kiện yêu cầu như nội lực, thể lực,... để sử dụng
                                skillInfo = autoUseTrainSkills.Where(x => !SkillManager.IsSkillCooldown(x.Key.ID) && SkillManager.IsAbleToUseSkill(x.Key, x.Value.Level, false)).FirstOrDefault();
                            }    
                          
                        }
                        else if (!KTAutoAttackSetting.IsTrainMode || IsTriger)
                        {
                            if (KTAutoAttackSetting._AutoConfig._AutoPKConfig.UseSkillByCombo)
                            {
                                /// Chọn 1 kỹ năng không trong trạng thái phục hồi và thỏa mãn điều kiện yêu cầu như nội lực, thể lực,... để sử dụng
                            ...
                            {
                                /// Chọn 1 kỹ năng không trong trạng thái phục hồi và thỏa mãn điều kiện yêu cầu như nội lực, thể lực,... để sử dụng
                                skillInfo = autoUsePKSkills.Where(x => !SkillManager.IsSkillCooldown(x.Key.ID) && SkillManager.IsAbleToUseSkill(x.Key, x.Value.Level, false)).FirstOrDefault();
                            }
                        ...
                        Int64 skillCastRange = skill.AttackRadius;  // BasicPropPA1Max -> int => Int64 no(int) 250110
                        ...
                        if (nextTarget == null || nextTarget.IsDeath || nextTarget.HP <= 0)
                        {
                            ...
                            if (this.TrainResetRadius.Elapsed.TotalSeconds > 10 && IsTrainMonster)
                            {
                                //Reset lại thời gian về bãi
                                this.TrainResetRadius = Stopwatch.StartNew();
                                // Nếu là đánh quanh điểm check xem có quá ranger không
                                if (KTAutoAttackSetting._AutoConfig._AutoTrainConfig.IsRadius)
                                {
                                    if (Vector2.Distance(leader.PositionInVector2, startPos) >= KTAutoAttackSetting._AutoConfig._AutoTrainConfig.Raidus)
                                    {
                                        ...
                                        KTGlobal.AddNotification("Quay về vị trí bãi vì chạy quá xa bán kinh đánh quái");
                                        KTLeaderMovingManager.AutoFindRoad(new Drawing.Point((int) startPos.x, (int) startPos.y));
                                        /// Bỏ qua Frame
                                        yield return new WaitForSeconds(this.AutoFight_TickSec);
                                        /// Tiếp tục vòng lặp
                                        continue;
                                ...
                                else
                                {
                                    ...
                                    nextTarget = this.FindBestTarget(null, false, (int)skillCastRange); // BasicPropPA1Max -> int => Int64 no(int) 250110
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            Int64 lastTargetHP = SkillManager.GetTargetTrueHP(this.AutoFightLastTarget);        // int default HP	back_if_error
                            ...
                            // Nếu là chế độ lure quái và đang trian monster thì sẽ đấm mỗi con 1 cái để lURE

                            if (KTAutoAttackSetting._AutoConfig._AutoTrainConfig.AttackMode == 2 && IsTrainMonster && IsCanNextTaget)
                            {
