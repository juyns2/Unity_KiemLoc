    ...
    public static partial class KTGlobal
    {
        ...
        public static Dictionary<KE_EQUIP_POSITION, GoodsData> GetEquips(RoleData roleData)
        {
            ...
            foreach (GoodsData equip in roleData.GoodsDataList)
            {
                switch (equip.Using)
                {
					...
					/// Chân Nguyên
                    case (int) KE_EQUIP_POSITION.emEQUIPPOS_CHANNGUYEN:
                    {
                        equips[(KE_EQUIP_POSITION) equip.Using] = equip;
                        break;
                    }
					/// Ngoại Trang
                    case (int) KE_EQUIP_POSITION.emEQUIPPOS_NGOAITRANG:
                    {
                        equips[(KE_EQUIP_POSITION) equip.Using] = equip;
                        break;
                    }
					/// Thần Binh
                    case (int) KE_EQUIP_POSITION.emEQUIPPOS_THANBINH:
                    {
                        equips[(KE_EQUIP_POSITION) equip.Using] = equip;
                        break;
                    }
        ...
        private static string GetEquipTypeString(KE_ITEM_EQUIP_DETAILTYPE equipType)
        {
            ...
            switch (equipType)
            {
				...
				case KE_ITEM_EQUIP_DETAILTYPE.equip_channguyen:
                {
                    ret = "Chân Nguyên";
                }
                break;
				
				case KE_ITEM_EQUIP_DETAILTYPE.equip_ngoaitrang:
                {
                    ret = "Ngoại Trang";
                }
                break;
				
				case KE_ITEM_EQUIP_DETAILTYPE.equip_thanbinh:
                {
                    ret = "Thần Binh";
                }
                break;
        ...
        private static string GetEquipPositionString(KE_EQUIP_POSITION inputType)
        {
            ...
            switch (inputType)
            {
				...
				case KE_EQUIP_POSITION.emEQUIPPOS_CHANNGUYEN:
                {
                    ret = "Chân Nguyên";
                    break;
                }
				case KE_EQUIP_POSITION.emEQUIPPOS_NGOAITRANG:
                {
                    ret = "Ngoại Trang";
                    break;
                }
				case KE_EQUIP_POSITION.emEQUIPPOS_THANBINH:
                {
                    ret = "Thần Binh";
                    break;
                }
        ...
        private static KE_EQUIP_POSITION GetEquipPositionByEquipType(KE_ITEM_EQUIP_DETAILTYPE equipType)
        {
            switch (equipType)
            {
				...
				case KE_ITEM_EQUIP_DETAILTYPE.equip_channguyen:
                {
                    return KE_EQUIP_POSITION.emEQUIPPOS_CHANNGUYEN;
                }
				case KE_ITEM_EQUIP_DETAILTYPE.equip_ngoaitrang:
                {
                    return KE_EQUIP_POSITION.emEQUIPPOS_NGOAITRANG;
                }
				case KE_ITEM_EQUIP_DETAILTYPE.equip_thanbinh:
                {
                    return KE_EQUIP_POSITION.emEQUIPPOS_THANBINH;
                }
                case KE_ITEM_EQUIP_DETAILTYPE.equip_meleeweapon:
                case KE_ITEM_EQUIP_DETAILTYPE.equip_rangeweapon:
                {
        ...
        private static string InitEquipGemProbs(string props, ItemData itemData, int series, int enhanceLevel, bool isactive_all_ornament = false, int index = 0)
        {
            StringBuilder contentBuilder = new StringBuilder();
            Dictionary<int, string> contentString = new Dictionary<int, string>();
            /// Nếu vật phẩm không tồn tại
            if (itemData == null)
            {
                return "";
...

            {
                ...
                if (equipProp.ListGem.Count > 0)
                {
                    //contentBuilder.AppendLine("<color=#057aff>Thuộc tính ngẫu nhiên:</color>");
                    int gemIndex = index + 1;
                    int idGem = equipProp.ListGem[index] != null ? equipProp.ListGem[index] : 0;
                    if (idGem == 0)
                    {
                        contentBuilder.AppendLine("Lỗ " + gemIndex + " chưa khảm");
                    }
                    else
                    {

                        GemData gemData = Loader.Loader.gemStone.gemData.Find(x => x.SourceID == idGem);

                        if (gemData != null)
                        {
                            if (!Loader.Loader.Items.TryGetValue(gemData.SourceID, out ItemData item))
                            {
                                contentBuilder.AppendLine("Lỗ " + gemIndex + " khảm đá " + idGem + " data không hợp lệ!");
                            }
                            contentBuilder.AppendLine("<color=#c900f6>" + item.Name + "</color><color=#2df600> (" + gemData.Level + "/10</color>)");
                            for (int j = 0; j < gemData.gemAttribute.Count; j++)
                            {
                                double VALUE0 = gemData.gemAttribute[j].AttrValue;
                                int VALUE1 = -1;
                                int VALUE2 = 0;
                                double _Value0 = VALUE0, _Value1 = VALUE1, _Value2 = 0;

                                if (PropertyDefine.PropertiesBySymbolName.TryGetValue(gemData.gemAttribute[j].AttrName, out PropertyDefine.Property property))
                                {
                                    string result = string.Format("<color=#2df600>" + property.Description + "</color>", VALUE0.AttributeToString(), Utils.Truncate(_Value1 / 18f, 1), _Value2);
                                    contentBuilder.AppendLine(result);
                                }
                                else
                                {
                                    KTDebug.LogWarning("Symbol Not Found :" + gemData.gemAttribute[j].AttrName);
                                }
                            }
                        }
                        else
                        {
                            contentBuilder.AppendLine("\nLỗ " + gemIndex + " khảm đá " + idGem + " data không hợp lệ!");
                        }
                    ...
                    if (equipProp.BasicPropCount > 0)
                    {
                        ...
                        if (emptyProps != null)
                        {
                            ...
                            for (int i = 0; i < equipProp.BasicPropCount; i++)
                            {
                                ...
                                Int64 propValue0 = equipProp.BasicPropValue[postionGet];    // BasicPropPA1Max -> int => Int64
                                Int64 propValue1 = equipProp.BasicPropValue[postionGet + 1];  // BasicPropPA1Max -> int => Int64
                                Int64 propValue2 = equipProp.BasicPropValue[postionGet + 2];  // BasicPropPA1Max -> int => Int64

                                Int64 value0 = 0, value1 = 0, value2 = 0;   // BasicPropPA1Max -> int => Int64
        ...
        /// <summary>
        /// Xây Tooltip các thuộc tính mặt nạ
        /// </summary>
        /// <param name="itemData"></param>
        private static string InitMaskProperty(ItemData itemData)
        {
            StringBuilder contentBuilder = new StringBuilder();
            ...
            contentBuilder.AppendLine("<color=#057aff>Thuộc tính mặt nạ:</color>");
        ...
        private static string InitHorseProperty(ItemData itemData)
        {
            ...
            {
                /// Duyệt danh sách thuộc tính ẩn
                contentBuilder.AppendLine("<color=#057aff>Thuộc tính cộng thêm:</color>");
                foreach (BasicProp prop in emptyProps)
                {
                    if (PropertyDefine.PropertiesBySymbolName.TryGetValue(prop.BasicPropType, out PropertyDefine.Property property))
                    {
        ...
        private static void BuildToolTipItemInfo(UISuperToolTip_Component uiSuperToolTip, GoodsData itemGD, List<KeyValuePair<string, Action>> buttons = null, ShopItem shopItem = null)
        {
            ...
            if (KTGlobal.IsItemEquip(item.Genre))
            {
                ...
                if (starLevelStruct != null)
                {
                    if (itemGD.Props != null && itemGD.Props != "ERORR")
                    {
                        byte[] base64Decode = Convert.FromBase64String(itemGD.Props);
                        ItemGenByteData equipProp = DataHelper.BytesToObject<ItemGenByteData>(base64Decode, 0, base64Decode.Length);
                        if (equipProp != null && equipProp.ListGem != null && equipProp.ListGem.Count > 0)
                        {
                            foreach (int idGem in equipProp.ListGem)
                            {
                                if (idGem > 0 && Loader.Loader.Items.TryGetValue(idGem, out ItemData itemGem))
                                {
                                    starLevelStruct.Value += itemGem.ItemValue;
                                }
                            }
                        }
                    }
                ...
                switch (itemType)
                {
                    ...
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_pendant:
                    {
                            if (itemGD.Props != null)
                            {
                                byte[] base64Decode = Convert.FromBase64String(itemGD.Props);
                                ItemGenByteData equipProp = DataHelper.BytesToObject<ItemGenByteData>(base64Decode, 0, base64Decode.Length);
                                if (equipProp.ListGem != null && equipProp.ListGem.Count > 0)
                                {
                                    /// contentBuilder.AppendLine("<color=#c900f6>*****Thuộc Tính Bảo Thạch*****</color>");
                                    int gemIndex = 0;
                                    int Acctive = 0;
                                    foreach (int idGem in equipProp.ListGem)
                                    {
                                        ...
                                        if (idGem == 0)
                                        {
                                            uiSuperToolTip.SetGem = gemIndex + "_" + idGem + "_";
                                        }
                                        else
                                        {
                                            uiSuperToolTip.SetGem = gemIndex + "_" + idGem + "_" + InitEquipGemProbs(itemGD.Props, item, itemGD.Series, itemGD.Forge_level, active_all_ornament, gemIndex);
                                            Acctive++;
                                        }
                                        gemIndex++;
                                    }
                                    contentBuilder.AppendLine(string.Format("<color=#057aff>Thuộc tính bảo thạch: ({0}/{1})</color>", Acctive, equipProp.ListGem.Count));
                                }
                            }
                            contentBuilder2.Append(KTGlobal.InitEquipBaseAttributes(itemGD.Props, item, itemGD.Series, itemGD.Forge_level));
                        ...
                        {
                            contentBuilder2.Append(hiddenProps);
                        }
                        contentBuilder2.Append(KTGlobal.InitActiveSuite(item));
                        contentBuilder2.Append(KTGlobal.InitEquipEnhance(itemGD, item));
                        break;
                    }
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_book:
                    {
                        contentBuilder2.Append(KTGlobal.InitBookProperty(itemGD, item, itemGD.Series, itemGD.Forge_level));
                        break;
                    }
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_horse:
                    {
                        contentBuilder2.Append(KTGlobal.InitHorseProperty(item));
                        break;
                    }
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_mask:
                    {
                        contentBuilder.Append(KTGlobal.InitMaskProperty(item));
                        break;
                    }
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_signet:
                    {
                    ...
                    case KE_ITEM_EQUIP_DETAILTYPE.equip_zhen:
                    {
                        contentBuilder2.Append(KTGlobal.InitZhenProperty(itemGD, item));
                        break;
                    }
                }
            }
            ...
            else
            {
                ...
                contentBuilder2.AppendLine(string.Format("{0} - {1}", lockString, KTGlobal.IsItemCanBeSold(item.BindType) ? "Có thể bán" : "Không thể bán"));
            }

            /// Nếu có thể đổi Ngũ Hành Hồn Thạch
            if (KTGlobal.IsEquipAbleToRefineIntoFS(itemGD))
            {
                contentBuilder2.AppendLine();
                /// Số lượng Ngũ Hành Hồn Thạch phân giải được
                int refineFSCount = KTGlobal.CalculateTotalFSByRefiningEquip(itemGD);
                contentBuilder2.AppendLine(string.Format("<color=#64ff0a>Phân giải thu được <color=yellow>{0} Ngũ Hành Hồn Thạch</color></color>", refineFSCount));
            }
            ...
            if (!string.IsNullOrEmpty(itemGD.Endtime))
            {
                ...
                if (endTime > DateTime.Now)
                {
                    ...
                    {
                        contentBuilder2.AppendLine(string.Format("<color=#ffee2e>Thời gian sử dụng còn <color=#ff42e0>{0}</color></color>", text));
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(item.Intro))
            {
                ...
                contentBuilder2.AppendLine(introStr);
            }
            //Hiện thông số bảo thạch
            if (item.Genre == 24)
            {
                string totalValueStr = (item.ItemValue / 10000f).ToString();
                /// Nếu không có dấu chấm động
                if (!totalValueStr.Contains('.'))
                {
                    totalValueStr += ".0000";
                }
                contentBuilder2.AppendLine(string.Format("<color=#05ffe6>Tài phú:</color> <color=#50ff29>{0}</color>", totalValueStr));
                GemData gemData = Loader.Loader.gemStone.gemData.Find(x => x.SourceID == item.ItemID);
                string[] equipTypes = gemData.Equip.Split('|');
                contentBuilder2.AppendLine("<color=#057aff>Trang bị có thể khảm:</color>");
                foreach (string equipType in equipTypes)
                {
                    contentBuilder2.AppendLine("<color=#fffb29>" + KTGlobal.GetEquipTypeString((KE_ITEM_EQUIP_DETAILTYPE)int.Parse(equipType)) + "</color>");

                }
                if (gemData != null)
                {
                    contentBuilder2.AppendLine("\n<color=#057aff>Thuộc tính bảo thạch:</color>");
                    for (int j = 0; j < gemData.gemAttribute.Count; j++)
                    {
                        double VALUE0 = gemData.gemAttribute[j].AttrValue;
                        int VALUE1 = -1;
                        ...
                        {
                            string result = string.Format("<color=#2df600>" + property.Description + "</color>", VALUE0.AttributeToString(), Utils.Truncate(_Value1 / 18f, 1), _Value2);
                            contentBuilder2.AppendLine(result);
                        }
                        else
                        {
                            KTDebug.LogWarning("Symbol Not Found :" + gemData.gemAttribute[j].AttrName);
            ...
            uiSuperToolTip.Content2 = contentBuilder2.ToString();
            uiSuperToolTip.Close = () => {
                uiSuperToolTip.Content = "";
                uiSuperToolTip.Content2 = "";
                uiSuperToolTip.SetGem = "";
        ...
        public static int CalculateEquipMaxEnhanceLevel(ItemData itemData)
        {
			...
			else
            {
                ...
                int equipQuality = itemData.QualityPrefix;
                //int equipSuiteID = itemData.SuiteID;
                //if (KTGlobal.IsRefinedEquip(itemData) || equipQuality == 7 && equipSuiteID != 0)
                //{
                //    nMax = 30;
                //}
                //else
                ...
                //    nMax = 16;
                //}
                if (equipQuality >= 6 || KTGlobal.IsRefinedEquip(itemData) || itemData.IsArtifact)
                {
                    nMax = 16;
                }
                else
