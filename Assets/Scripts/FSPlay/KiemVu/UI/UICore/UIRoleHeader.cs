...
namespace FSPlay.KiemVu.UI.CoreUI
{
    ...
    public class UIRoleHeader : TTMonoBehaviour
    {
		...
		/// <summary>
        /// Ảnh danh hiệu nhân vật
        /// </summary>
        [SerializeField]
        private UIAnimatedSprite UIImage_RoleTitle;
        ...
        /// % máu
        /// </summary>
        public Int64 HPPercent		// int default HP
        {
            get
            {
                return (Int64) (this.UISlider_HPBar.value * 100);		// int default HP
            }
            set
            {
        ...
        public int CurrentRoleTitle
		{
			...
			set
			{
				...
				KTitleXML titleInfo = Loader.Loader.RoleTitles.Where(x => x.ID == value).FirstOrDefault();
				
				if (titleInfo == null)
				{
                    // Không tìm thấy danh hiệu, ẩn hình ảnh và xóa văn bản
					this._CurrentRoleTitle = -1;
					this.UIImage_RoleTitle.gameObject.SetActive(false);
					this.UIText_RoleTitle.text = "";
                    /// Xây lại giao diện
                    this.RebuildLayout(this.UIText_RoleTitle.transform.parent.GetComponent<RectTransform>());
					this.RebuildLayout(this.UIImage_RoleTitle.transform.parent.GetComponent<RectTransform>());
                }
                /// Nếu tìm thấy
				else
				{
                    // Nếu tìm thấy danh hiệu có ảnh thì hiển thị ảnh
            		if (!string.IsNullOrEmpty(titleInfo.BundleDir) && !string.IsNullOrEmpty(titleInfo.AtlasName) && titleInfo.SpriteNames.Count > 0)
            		{
                        //this.UIImage_RoleTitle.AutoPlay = false;
                        this.UIImage_RoleTitle.BundleDir = titleInfo.BundleDir;
            		    this.UIImage_RoleTitle.AtlasName = titleInfo.AtlasName;
            		    this.UIImage_RoleTitle.SpriteNames = titleInfo.SpriteNames.ToArray();
            		    this.UIImage_RoleTitle.Duration = titleInfo.AnimationSpeed;
            		    this.UIImage_RoleTitle.PixelPerfect = true;
            		    this.UIImage_RoleTitle.Scale = titleInfo.Scale;
            		    this.UIImage_RoleTitle.gameObject.SetActive(true);
            		    this.UIImage_RoleTitle.Play();
						this.UIText_RoleTitle.text = "";
						/// Xây lại giao diện
						this.RebuildLayout(this.UIText_RoleTitle.transform.parent.GetComponent<RectTransform>());
						this.RebuildLayout(this.UIImage_RoleTitle.transform.parent.GetComponent<RectTransform>());
            		...
            		{
            		    // Nếu không có hình ảnh thì hiện text
            		    this.UIImage_RoleTitle.gameObject.SetActive(false);
						this.UIText_RoleTitle.text = titleInfo.Text;
						/// Xây lại giao diện
						this.RebuildLayout(this.UIText_RoleTitle.transform.parent.GetComponent<RectTransform>());
						this.RebuildLayout(this.UIImage_RoleTitle.transform.parent.GetComponent<RectTransform>());
            		}
