...
namespace FSPlay.KiemVu.UI.Main.SuperToolTip
{
    ...
    public class UISuperToolTip_Component : MonoBehaviour
    {
        ...
        [SerializeField]
        private TextMeshProUGUI UIText_Content2;
        [SerializeField]
        private List<UIItemBox> UI_GemBox;
        [SerializeField]
        private List<TextMeshProUGUI> UI_GemDes;
        ...
        public string Content2
        {
            get
            {
                return this.UIText_Content2.text;
            }
            set
            {
                this.UIText_Content2.text = value;
            }
        }
        public string SetGem
        {
            get
            {
                return null;
            ...
            set
            {
                ...
                if (data.Length == 3)
                {
                    ...
                    int idItem = int.Parse(data[1]);
                    string des = data[2];
                    if (Loader.Loader.Items.TryGetValue(idItem, out ItemData itemData) && UI_GemBox[index] != null)
                    {
                        GoodsData itemGD = KTGlobal.CreateItemPreview(itemData);
                        /// Thêm vào danh sách
                        UI_GemBox[index].Data = itemGD;
                        UI_GemDes[index].text = des;
                        UI_GemBox[index].transform.parent.gameObject.SetActive(true);
                        UI_GemBox[index].Click = null;
                    }
                    else
                    {
                        UI_GemBox[index].Data = null;
                        UI_GemDes[index].text = "Chưa khảm đá";
                        UI_GemBox[index].transform.parent.gameObject.SetActive(true);
                        UI_GemBox[index].Click = null;
                    }
                }
                ...
                {
                    //đóng hết các object gem
                    foreach (UIItemBox item in UI_GemBox)
                    {
