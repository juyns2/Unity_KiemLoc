    ...
    public static partial class SkillManager
    {
        ...
        public static bool ValidateUseSkill(SkillDataEx skillData, bool notification = true)
        {
            ...
            if (skillData.WeaponLimit != (int) KE_EQUIP_WEAPON_CATEGORY.emKEQUIP_WEAPON_CATEGORY_ALL)
            {
                    ...
                    if (weapon != null)
                    {
                        if (Loader.Loader.Items.TryGetValue(weapon.GoodsID, out ItemData itemData))
                        {
                            ...
                            if (itemData.Category != skillData.WeaponLimit)
                            {
                                if (notification)
                                {
                                    KTGlobal.AddNotification("Kỹ năng này yêu cầu vũ khí tay không hoặc " + KTGlobal.GetWeaponKind(skillData.WeaponLimit) + "!");
                                }
                                return false;
                            }
                        }
                ...
                else
                {
                    if (weapon == null)
                    {
                        if (notification)
                        {
                            KTGlobal.AddNotification("Kỹ năng này yêu cầu vũ khí " + KTGlobal.GetWeaponKind(skillData.WeaponLimit) + "!");
                        }
                        return false;
                    }
                    else
            ...
            if (skillData.FactionID != 0)
            {
                ...
                else if (skillData.SubID != 0 && Global.Data.RoleData.SubID != skillData.SubID)
                {
                    if (notification)
                    {
                        KTGlobal.AddNotification("Hệ phái không phù hợp!");
                    }
                    return false;
                }
            }
        ...
        public static bool IsAbleToUseSkill(SkillDataEx skillData, int level, bool notification = true)
        {
            if (skillData.Properties.TryGetValue((byte) level, out PropertyDictionary levelPd))
            {
                ...
                {
                    /// Số điểm cần
                    Int64 pointNeed = levelPd.Get<KMagicAttrib>((int) MAGIC_ATTRIB.magic_skill_cost_v).nValue[0];   // BasicPropPA1Max -> int => Int64 no(int) 250110
        ...
        public static bool LeaderUseSkill(int skillID, bool findNearestTarget = false, bool ignoreTarget = false, bool silentMode = false, bool isSubPhrase = false)
        {
            ...
            //    KTGlobal.AddNotification("Bản đồ hiện tại không cho phép dùng kỹ năng!");
            //    return false;
            //}
            ///// Nếu bản đồ cấm dùng kỹ năng này
            //else if (Global.Data.GameScene.CurrentMapData.Setting.BanSkills.Contains(skillID))
            //{
            //    /// Đánh dấu đang đợi dùng kỹ năng
            //    SkillManager.IsWaitingToUseSkill = false;

            //    KTGlobal.AddNotification("Bản đồ hiện tại không cho phép dùng kỹ năng này!");
            //    return false;
            //}
            ...
            ///// Nếu đang chạy thì đứng lại 1 lát rồi mới cho dùng kỹ năng
            ...
            //        SkillManager.LeaderUseSkill(skillID, findNearestTarget);
            //    });
            //    return false;
            //}

            ///// Nếu thời gian lần cuối thực hiện di chuyển chưa Delay đủ
            //if (KTGlobal.GetCurrentTimeMilis() - KTLeaderMovingManager.LastMoveTick < KTGlobal.RunningWaitToUseSkill * 1000)
            //{
            //    /// Đánh dấu đang đợi dùng kỹ năng
            //    SkillManager.IsWaitingToUseSkill = true;

            //    IEnumerator AutoRefreshWaitingToUseSkill()
            //    {
            //        yield return new WaitForSeconds(KTGlobal.RunningWaitToUseSkill);
            //        /// Đánh dấu đang đợi dùng kỹ năng
            //        SkillManager.IsWaitingToUseSkill = false;
            //        /// Thực hiện gọi lại hàm
            //        SkillManager.LeaderUseSkill(skillID, findNearestTarget, ignoreTarget, silentMode, isSubPhrase);
            //    }
            //    PlayZone.GlobalPlayZone.StartCoroutine(AutoRefreshWaitingToUseSkill());
            //    return false;
            //}
            ...
            {
                skillCastRange = (int)skillPd.Get<KMagicAttrib>((int) MAGIC_ATTRIB.magic_skill_attackradius).nValue[0]; // BasicPropPA1Max -> int => Int64 no(int) 250110
            }
            ...
            if (target != null)
            {
                ...
                PlayZone.GlobalPlayZone.NotifyRoleFace(target.RoleID, -1, true);
        ...
        public static void SpriteUseSkill(GSprite sprite, Direction direction, int skillID, bool isSpecialAttack)
        {
            ...
            if (Loader.Loader.Skills.TryGetValue(skillID, out SkillDataEx skill))
            {
                ...
                else if (character != null)
                {
                    ...
                    {
                        PlayZone.GlobalPlayZone.StartCoroutine(KTResourceManager.Instance.LoadAssetAsync<AudioClip>(Loader.Loader.SkillCastSoundBundleDir, soundName, false, (isNewLoad) => {
        ...
        public static void SpriteBlinkToPosition(GSprite sprite, Vector2 position, float duration, bool isDragged = false)
        {
            ...
            sprite.FlyingCoroutine = PlayZone.GlobalPlayZone.StartCoroutine(DoPlayAnimation());
        }
